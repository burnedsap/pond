class World {
  ...
  void run() {
    ...
    for (int i=0; i<bg.size(); i++) {
      ...
      if ((part.hunger<4)) {
        //if ((dist(target.x, target.y, part.loc.x, part.loc.y)<100)&&(part.hunger<4)) {
        //if (dist(target.x, target.y, part.loc.x, part.loc.y)<70) {
        fill(0, 0, 255);
        text("eat", part.loc.x, part.loc.y+12);
        stroke(51, 50);
        line(part.loc.x, part.loc.y, target.x, target.y);
        part.arrive(target);
