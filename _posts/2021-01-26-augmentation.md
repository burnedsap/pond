...
The addition of visual graphs provided a vastly quicker method of understanding the trends of each entity, which accelerated the development. The live telemetry was also very useful when debugging certain issues. For instance, the plant logger would show a high number of plants, but there weren’t any plants seen on screen itself, and as a result the Opus would quickly die out. Even though the numbers showed that there were more than enough plants, they had no impact on the system. Eventually, after a while it became clear that the plants were generating outside the set canvas, with the entities unable to access them. A quick fix later, and the problem was rectified. 

![SL5_1test](/pond/media/sl5_1test.png)
...
Video of ecosystem
![pondmovie](/pond/media/pondmovie.mov)
